@extends('layouts.adm.app', [
    'wsecond_title' => 'Keuangan: Harian (Tahun: '.$year.' / Bulan: '.formatBulan($month).')',
    'sidebar_menu' => 'accounting',
    'sidebar_submenu' => null,
    'wheader' => [
        'header_title' => 'Keuangan: Bulanan (Tahun: '.$year.' / Bulan: '.formatBulan($month).')',
        'header_breadcrumb' => [
            [
                'title' => 'Dashboard',
                'is_active' => false,
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <table class="table table-hover table-striped table-bordered" id="accounting_daily-table">
            <thead>
                <tr>
                    <th>Tanggal</th>
                    <th>Nominal</th>
...
<script>
    ...
    const getChartData = () => {
        $.get(`{{ route('adm.json.datatable.accounting.daily', [$year, $month]) }}`, (result) => {
    ...
    const buildChart = (data) => {
        // Empty Container
        $("#daily-chart").remove();
        $("#canvas-holder").append(`<canvas id="daily-chart"><canvas>`);
        ...
        if(data.length > 0){
            data.forEach((e) => {
                ...
                // Push Year to Label
                labelsData.push(moment(e.date).format('D MMMM, YYYY'));
                // Get Temp Data
        ...
        // Chart.Js init
        let lineChart = new Chart(document.getElementById("daily-chart"), {
            type: 'line',
            data: chartData,
    ...
    $(document).ready((e) => {
        $("#accounting_daily-table").DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('adm.json.datatable.accounting.daily', [$year, $month]) }}",
                type: "GET",
                data: (d) => {
            ...
            columnDefs: [
                {
                    ...
                    "render": (row, type, data) => {
                        return `
                            <div class="btn-group">
                                <a href="{{ route('adm.accounting.daily', [$year, $month]) }}/${data.date}" class="btn btn-sm btn-primary btn-action">
        ...
        $("#input-store_id").change((e) => {
            $("#accounting_daily-table").DataTable().ajax.reload();
            updateFilter();
        });
