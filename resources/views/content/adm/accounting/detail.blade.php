@extends('layouts.adm.app', [
    'wsecond_title' => 'Keuangan: Detail ('.date("d F, Y", strtotime($date)).')',
    'sidebar_menu' => 'accounting',
    'sidebar_submenu' => null,
    'wheader' => [
        'header_title' => 'Keuangan: Detail ('.date("d F, Y", strtotime($date)).')',
        'header_breadcrumb' => [
            [
                'title' => 'Dashboard',
                'is_active' => false,
                ...
                'title' => 'Bulan: '.formatBulan($month),
                'is_active' => false,
                'url' => route('adm.accounting.monthly', $month)
            ], [
                'title' => 'Keuangan: '.date("d F, Y", strtotime($date)),
                'is_active' => true,
                'url' => null
            ],
        ]
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <table class="table table-hover table-striped table-bordered" id="accounting_hourly-table">
            <thead>
                <tr>
                    <th>Jam</th>
                    <th>Nominal</th>
...
<script>
    ...
    const getChartData = () => {
        $.get(`{{ route('adm.json.datatable.accounting.detail', [$year, $month, $date]) }}`, (result) => {
    ...
    const buildChart = (data) => {
        // Empty Container
        $("#hourly-chart").remove();
        $("#canvas-holder").append(`<canvas id="hourly-chart"><canvas>`);
        ...
        // Chart.Js init
        let lineChart = new Chart(document.getElementById("hourly-chart"), {
            type: 'line',
            data: chartData,
    ...
    $(document).ready((e) => {
        $("#accounting_hourly-table").DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('adm.json.datatable.accounting.detail', [$year, $month, $date]) }}",
                type: "GET",
                data: (d) => {
            ...
            columnDefs: [
                {
                    ...
                    "render": (row, type, data) => {
                        ...
                        return `
                            ...
                            <hr class="my-2"/>
                            <a href="{{ route('adm.transaction.index') }}/${data.uuid}" class="btn btn-sm btn-primary"><i class="fas fa-eye mr-2"></i>Detail</a>
                        `;
                    }
                }
            ]
        ...
        $("#input-store_id").change((e) => {
            $("#accounting_hourly-table").DataTable().ajax.reload();
            updateFilter();
        });
