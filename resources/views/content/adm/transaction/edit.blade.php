@extends('layouts.adm.app', [
    ...
    'wsecond_title' => 'Transaksi: Edit Data',
    'sidebar_menu' => 'transaction',
    'sidebar_submenu' => null,
    'wheader' => [
        'header_title' => 'Transaksi: Edit Data',
        'header_breadcrumb' => [
            [
                'title' => 'Dashboard',
                'is_active' => false,
...
@section('content')
<form class="card" method="POST" action="{{ route('adm.transaction.update', $data->uuid) }}" enctype="multipart/form-data" id="transaction-form">
    @csrf
    @method('PUT')

    <div class="card-header">
        <h3 class="card-title">Transaksi: Edit Data</h3>
        ...
        <div class="row">
            <div class="col-12 col-lg-6">
                ...
                <div class="form-group">
                    <label>Toko</label>
                    <select class="form-control @error('store_id') is-invalid @enderror" id="input-store_id" name="store_id" style="width: 100% !important;" disabled>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('customer_id') is-invalid @enderror" id="input-customer_id" name="customer_id" style="width: 100% !important;" disabled>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control form-select2 @error('status') is-invalid @enderror" id="input-status" name="status" style="width: 100% !important;">
                        @foreach ($status as $item)
                            <option value="{{ $item['value'] }}" {{ $data->status == $item['value'] ? 'selected' : '' }}>{{ $item['name'] }}</option>
                        @endforeach
                    </select>
                    @error('type')
                    <span class="invalid-feedback">{{ $message }}</span>
                ...
                <div class="form-group">
                    <label>Catatan</label>
                    <textarea class="ckeditor d-none @error('note') is-invalid @enderror" name="note" id="input-note" placeholder="Catatan mengenai Transaksi akan berada disini...">{!! $data->note !!}</textarea>
                    @error('note')
                    <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div id="product-container">
                            @if ($data->transactionItem()->exists())
                                @foreach ($data->transactionItem as $key => $item)
                                    <div class="product-item" data-loop="{{ $key }}">
                                        <input type="hidden" name="product[{{ $key }}][validate]" value="{{ $item->id }}">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-12 col-lg-6">
                                                <label>SN</label>
                                                <select class="form-control form-product-sn @error('product.'.$key.'.sn_id') is-invalid @enderror" id="input_{{ $key }}-sn_id" name="product[{{ $key }}][sn_id]" style="width: 100% !important;">
                                                    <option value="{{ $item->productDetail->id }}" selected>{{ $item->productDetail->serial_number }}</option>
                                                </select>
                                                @error('product.'.$key.'.sn_id')
                                                <span class="invalid-feedback">{{ $message }}</span>
                                                @enderror

                                                <small class="text-muted d-none product-needed">*Harap pilih produk terlebih dahulu</small>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-6">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input data-inputmask="'alias': 'decimal', 'groupSeparator': ',', 'digits': '0', 'allowMinus': 'false'" name="product[{{ $key }}][price]" id="input_{{ $key }}-price" class="form-control product-price @error('product.'.$key.'.price') is-invalid @enderror" value="{{ $item->price }}" min="0" placeholder="Biaya Sewa" min="0" onchange="updatePrice()" required>
                                                </div>
                                            ...
                                            <div class="form-group col-6">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input data-inputmask="'alias': 'decimal', 'groupSeparator': ',', 'digits': '0', 'allowMinus': 'false'" name="product[{{ $key }}][discount]" id="input_{{ $key }}-discount" class="form-control product-discount @error('product.'.$key.'.discount') is-invalid @enderror" value="{{ $item->discount }}" min="0" placeholder="Diskon Sewa" min="0" onchange="updateDiscount()" required>
                                                </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <textarea class="form-control @error('product.'.$key.'.note') is-invalid @enderror" placeholder="Catatan Produk" name="product[{{ $key }}][note]">{!! $item->note !!}</textarea>
                        ...
                        <hr/>
                        <button type="button" class="btn btn-sm btn-primary" id="btn_product-add_more" {{ $data->store()->exists() ? '' : 'disabled' }}><i class="fas fa-plus mr-2"></i>Tambah lainnya</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col-12 col-lg-6">
                ...
                <div class="form-group row align-items-center">
                    ...
                    <div class="col-sm-8">
                        <div class="input-group">
                            ...
                            <input data-inputmask="'alias': 'decimal', 'groupSeparator': ',', 'digits': '0', 'allowMinus': 'false'" name="extra_amount" id="input-extra_amount" class="form-control" value="{{ $data->extra }}" min="0" placeholder="Biaya Tambahan" min="0" onchange="calculateSumAmount()">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        ...
                        <small class="text-muted">(Jumlah Biaya - Jumlah Diskon) * Lama Sewa</small>
                    </div>
                </div>
                <div class="form-group row align-items-center">
                    <label class="col-sm-4 col-form-label">
                    ...
                    <div class="col-sm-8">
                        <div class="input-group">
                            ...
                            <input data-inputmask="'alias': 'decimal', 'groupSeparator': ',', 'digits': '0', 'allowMinus': 'false'" name="paid" id="input-paid" class="form-control" value="{{ $data->paid }}" min="0" placeholder="Biaya dibayarkan" min="0" onchange="calculateLeftOver()">
                        </div>
                    </div>
                </div>
                <hr/>
...
<script>
    ...
    $(document).ready((e) => {
        ...
        $('input[name="daterange"]').daterangepicker({
            startDate: "{{ date("m/d/Y H:i", strtotime($data->start_date)) }}",
            endDate: "{{ date("m/d/Y H:i", strtotime($data->end_date)) }}",
            timePicker: true,
